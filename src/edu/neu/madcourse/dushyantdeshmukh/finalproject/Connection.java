...
public class Connection extends Activity implements OnClickListener{
	  ...
	  protected void onStart() {
	    ...
	    String userNameValue = projPreferences.getString(ProjectConstants.USER_NAME, null);
	  ...
	  private void saveUserNameToRegisteredUserList(String userName, String regId) {
		  if (!InternetConnUtil.isNetworkAvailable(context)) {
		      Util.showToast(context, Constants.NETWORK_UNAVAILABLE_MSG, Toast.LENGTH_LONG);
		      return;
		  }
		  
		  Util.addValuesToKeyOnServer(ProjectConstants.REGISTERED_USERS_LIST,userName,regId);
	    ...
	    new AsyncTask<String, Integer, String>() {
	      ...
	      protected String doInBackground(String... params) {
	        ...
	        if (KeyValueAPI.isServerAvailable()) {
	          String registeredUsersList = KeyValueAPI.get(ProjectConstants.TEAM_NAME,
	        		  ProjectConstants.PASSWORD, ProjectConstants.REGISTERED_USERS_LIST);
	                ...
	                if (oppUsername.equalsIgnoreCase(oppName)) {
	                  ...
	                  try {
	                    result = Util.sendPost("data." + Constants.KEY_MSG_TYPE
	                        + "=" + ProjectConstants.MSG_TYPE_FP_CONNECT + "&data."
	                        + ProjectConstants.KEY_REG_ID + "=" + regId + "&data."
	                        + ProjectConstants.KEY_USERNAME + "=" + username, oppRegId);
	  ...
	  protected void handleOpponentResponse(String data) {
	    ...
	    if (dataMap.containsKey(Constants.KEY_MSG_TYPE)) {
	      ...
	      if (msgType.equals(ProjectConstants.MSG_TYPE_FP_CONNECT)) {
	        ...
	        projPreferences.edit().putString(ProjectConstants.PREF_OPPONENT_REG_ID, this.oppRegId).commit();
...
//	        displayMsg("Setting this.oppRegId in SP: " + this.oppRegId);
	        ...
	        showAcceptRejectDialog();
	    	  ...
	    	  Log.d(TAG, "Inside MSG_TYPE_FP_ACK_ACCEPT = " + ProjectConstants.MSG_TYPE_FP_ACK_ACCEPT);
	        this.oppRegId = dataMap.get(ProjectConstants.KEY_REG_ID);   
	        projPreferences.edit().putString(ProjectConstants.PREF_OPPONENT_REG_ID, this.oppRegId).commit();
	        Log.d(TAG, "\n\n Setting this.oppRegId in SP: " + this.oppRegId + "\n\n");
...
//	        String opponentName = dataMap.get(Constants.KEY_USERNAME);
	        Util.showToast(context,"Game request denied by user '" + oppName + "'.", Toast.LENGTH_LONG);
	      }
	    }
	  }
...
//	      send an reject ack to oppponent
	  ...
	  + (accepted? ProjectConstants.MSG_TYPE_FP_ACK_ACCEPT : ProjectConstants.MSG_TYPE_FP_ACK_REJECT));
	    new AsyncTask<String, Integer, String>() {
	      @Override
	      protected String doInBackground(String... params) {
	        String retVal;
	        ...
	        try {
	          retVal = Util.sendPost("data." + Constants.KEY_MSG_TYPE
	              + "=" + (accepted? ProjectConstants.MSG_TYPE_FP_ACK_ACCEPT : ProjectConstants.MSG_TYPE_FP_ACK_REJECT) 
	              + "&data." + ProjectConstants.KEY_REG_ID + "=" + regId + "&data."
	              + ProjectConstants.KEY_USERNAME + "=" + username, oppRegId);
...
//	        t.show();
	  ...
	  private String getRegistrationId(Context context) {
	        String registrationId = projPreferences.getString(ProjectConstants.PROPERTY_REG_ID, "");
	        ...
	        // app version.
	        int registeredVersion = projPreferences.getInt(ProjectConstants.PROPERTY_APP_VERSION, Integer.MIN_VALUE);
	        ...
	        if (resultCode != ConnectionResult.SUCCESS) {
	            if (GooglePlayServicesUtil.isUserRecoverableError(resultCode)) {
	                GooglePlayServicesUtil.getErrorDialog(resultCode, this,
	                        ProjectConstants.PLAY_SERVICES_RESOLUTION_REQUEST).show();
	            } else {
