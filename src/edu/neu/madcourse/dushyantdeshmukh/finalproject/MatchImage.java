...
public class MatchImage extends BaseCameraActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View matchViewControl = controlInflater.inflate(
				R.layout.final_proj_match, null);
  ...
  private void renderImgToMatch(int imgIndex) {
	...
	/**
	 * Given the index of currently matched image, returns the index of next
	 * image to be matched
	 * 
	...
	protected void processCapturedPicture(byte[] data) {
		...
		if (Util.imagesMatch(imgMat1, imgMat2)) {
			...
			String oppRegId = projPreferences.getString(
			...
			if (imagesMatched == totalNoOfImgs) {
				...
				editor.putString(ProjectConstants.PLAYER_IMAGE_COUNT,
						Integer.toString(imagesMatched));
				editor.commit();

				Log.d(TAG, "PLAYER TIME: " + timeElapsed
						+ "PLAYER_IMAGE_COUNT: " + imagesMatched);

				sendGameMoveOrFinishToOpponent(false, oppRegId, imagesMatched,
						timeElapsed);
	...
	protected void handleOpponentResponse(String data) {
		...
		if (dataMap.containsKey(Constants.KEY_MSG_TYPE)) {
			...
			if (msgType.equals(ProjectConstants.MSG_TYPE_FP_GAME_OVER)) {
				...
				editor.putString(ProjectConstants.OPPONENT_TIME,
						opponent_matchingTime);
				...
				Util.showToast(this, "Opponent matched new Image!",
						Toast.LENGTH_LONG);
	...
	private void sendGameMoveOrFinishToOpponent(boolean isGameMoveEvent,
			String oppRegId, int imagesMatched, int timeForMatching) {
			...
			protected String doInBackground(String... params) {
				...
				try {
					retVal = Util.sendPost("data."
							...
							+ imagesMatched
							+ (isGameMove ? "" : "&data."
									+ ProjectConstants.TOTAL_MATCHING_TIME
									+ "=" + matchingTime), oppRegId);
		...
		}.execute(String.valueOf(isGameMoveEvent),
				String.valueOf(imagesMatched), String.valueOf(timeForMatching),
				oppRegId, null);
	}
